...
def extract_line(image, beta=0.7, alpha=0.002, show = True):
	...
	#Binary thresholding and inverting at 127 TODO: change this
	th, threshed = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY_INV|cv2.THRESH_OTSU)
	# th, threshed = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV|cv2.THRESH_OTSU,11,2)
...
def get_final_expression(image_path, acc_thresh=0.60):
	'''
...
#            print('Lines in workspace-%d : %d' %(r, len(y1s)))
	...
	for bn in box_nums:
		...
		# print(df['char_df']['pred'])
		# print(dir(df['char_df']))
		for box in df['char_df']:
			new_line = ''
			for char in box['pred']:
				new_line += char
